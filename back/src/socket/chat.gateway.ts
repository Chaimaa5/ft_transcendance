...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect{
    ...
    async handleMessage(client: Socket, body : {roomId: string, message: string}){
        ...
        if(!isMuted && !isBanned){
            const sendMessage = await this.chatService.storeMessage(room, userId, message);
            ...
            if(ChatRoom){
                ...
                ChatRoom.forEach(socket =>{
                    this.server.to(socket.id).emit('receiveMessage', message)
                })
            }

            // this.server.to(roomId).except(client.id).emit('message', message);
            
            // this.server.to(roomId).emit('message', sendMessage);
        }
    }
